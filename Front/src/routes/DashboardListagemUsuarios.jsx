...
const excluirUsuario = async (id) => {
    try {
        const response = await fetch(`http://localhost:3600/ExcluirUsuario/${id}`, {
            method: 'DELETE',
            headers: {
...
function App() {
    ...
    useEffect(() => {
        const fetchProducts = async () => {
            try {
                const response = await fetch("http://localhost:3600/feedusuarios");
                const data = await response.json();
                setproduto(data);
            } catch (error) {
                        ...
                        <tbody>
                            {produto.map((item) => (
                                ...
                                <tr key={item.id} className="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                                    ...
                                    <td className="px-6 py-4">
                                        {/*        função anonima que é executada ao clickar no botao */}
